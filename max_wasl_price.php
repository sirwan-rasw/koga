...
if(isset($_POST["key"]))
{
    ...
    {   
        ...
        $sql="SELECT SUM(price),id_koga,id_comp,date FROM sales,companies,stores WHERE state=1 AND id_comp='$id_comp' AND wasl=1 AND sales.id_comp=companies.id AND sales.id_koga=stores.id and type_price='$se' GROUP BY id_koga";
        ...
        <tr style="background-color: darkCyan;  font-size: 15px;" align="center">
            ...
            <td><?php echo $row["date"];  ?></td>
                    ...
                    $sql="SELECT SUM(price),id_koga,id_comp,date FROM sales,companies,stores WHERE state=1 AND id_koga='$id_koga' AND wasl=1 AND sales.id_comp=companies.id AND sales.id_koga=stores.id and type_price='$se' GROUP BY id_comp";
        $res=mysqli_query($conn,$sql);
